                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    <label for="address" class="control-label">Recipient's Public Address</label>
                                    <input type="text" v-model="address" disabled class="form-control" placeholder="Recipient's public address" >
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    methods:{
        ...
        makeTransaction(){
            var self = this;
            swal('Please Wait','Creating Transaction...', {
            ...
            if(!this.address) {
                swal('Error!', 'The recipient\'s public address is not set!');
                return;
            }
        ...
        axios.post(`{{ route('coinpayment.ajax.create.withdrawal') }}`, params)
        .then(function(json){
                console.log(json);
        ...
        .then(function(json){
                axios.post('/withdrawal/approve/' + JSON.parse(payload).details.withdrawal_info_id, {})
                .then(function(res) {
                    console.log('just saved the withdrawal info', res);
                });
                ...
                $('#paynow').modal('show');
                    ...
                    .catch(function(error){
                        swal('Danger!', 'Whoops! Looks like something went wrong');
                    });

        }else{
            swal('Danger!', `${json.data.error}!`);
        }
    })
    .catch(function(err){
        if(err !== undefined)
